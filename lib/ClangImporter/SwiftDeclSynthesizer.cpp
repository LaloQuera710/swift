...
synthesizeDefaultArgumentBody(AbstractFunctionDecl *afd, void *context) {
  ...
  auto clangDeclName = clang::DeclarationName(
      &clangCtx.Idents.get(("__cxx_" + funcDecl->getNameStr()).str()));
  ...
  // Synthesize `ParamTy __defaultArg_XYZ() { return {default expr}; }`.
  ...
  // Import `func __defaultArg_XYZ() -> ParamTY` into Swift.
  ...
  // Synthesize a call to `__defaultArg_XYZ()`.
  ...
  // Synthesize `return __defaultArg_XYZ()`.
